...
SimIO::SimIO(uint8_t update_rate_hz, IIOCompleteNotification* notify_sink,
  ...
  if (sim_device) {
    ...
    this->simConnected =
        sim_device->CreateBoolean("Connected", direction, true);
    ...
    this->simCompassHeading =
        sim_device->CreateDouble("CompassHeading", direction, 0.0f);
    this->simFusedHeading =
        sim_device->CreateDouble("FusedHeading", direction, 0.0f);

    this->simLinearWorldAccelX =
        sim_device->CreateDouble("LinearWorldAccelX", direction, 0.0f);
    this->simLinearWorldAccelY =
        sim_device->CreateDouble("LinearWorldAccelY", direction, 0.0f);
    this->simLinearWorldAccelZ =
        sim_device->CreateDouble("LinearWorldAccelZ", direction, 0.0f);
    ...
    board_state.sensor_status =
        // NAVX_SENSOR_STATUS_MOVING |              // NOTE:  Updated by Sim
        // Variable
    ...
    board_state.cal_status = NAV6_CALIBRATION_STATE_COMPLETE;
    ...
    ahrs_update.barometric_pressure =
        1013.25f;  // Nominal pressure at earth's surface
    ...
    // The following values are updated by Sim Variables later; set to
    ...
    std::printf("navX-Sensor SimDevice created.\n");
  }
}
...
#define SIM_BYTES_PER_SECOND 1000
  double num_secs_running =
      unit_cast<double>(Timer::GetFPGATimestamp()) - start_seconds;
...
void SimIO::Run() {
  ...
  long sensor_timestamp =
      2000;  // NOTE:  Simulate a 2-second navX-sensor firmware startup delay

  // Update all static values
  notify_sink->SetBoardID(board_id);
  notify_sink->SetRawData(raw_data_update, sensor_timestamp);

  // Update AHRS data (portions of which are static; others are updated from sim
  // variables)
...
float SimIO::CalculateNormal(float in1, float in2) {
  return fabs(std::sqrt(std::fabs(in1 * in1) + std::fabs(in2 * in2)));
}
...
void SimIO::UpdatePeriodicFromSimVariables(long sensor_timestamp) {
  ...
  if (curr_is_connected) {
    ...
    // Detect motion
    float last_linear_accel_norm =
        CalculateNormal(last_linear_world_accel_x, last_linear_world_accel_y);
    float curr_linear_accel_norm =
        CalculateNormal(ahrs_update.linear_accel_x, ahrs_update.linear_accel_y);
    bool curr_is_moving =
        (std::fabs(curr_linear_accel_norm - last_linear_accel_norm) >= 0.02f);
    if (curr_is_moving) {
    ...
    // Detect rotation
    bool curr_is_rotating = std::fabs(last_yaw - ahrs_update.yaw) >= 0.05f;
    if (!curr_is_rotating) {
