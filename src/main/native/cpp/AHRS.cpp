...
class AHRSInternal : public IIOCompleteNotification, public IBoardCapabilities {
  AHRS* ahrs;
  friend class AHRS;
  AHRSInternal(AHRS* ahrs) { this->ahrs = ahrs; }
  ...
  void SetAHRSPosData(AHRSProtocol::AHRSPosUpdate& ahrs_update,
    ...
    ahrs->is_moving =
        (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_MOVING) != 0)
             ? true
             : false);
    ahrs->is_rotating =
        (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_YAW_STABLE) != 0)
             ? false
             : true);
    ahrs->altitude_valid =
        (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_ALTITUDE_VALID) != 0)
             ? true
             : false);
    ahrs->is_magnetometer_calibrated =
        (((ahrs_update.cal_status & NAVX_CAL_STATUS_MAG_CAL_COMPLETE) != 0)
             ? true
             : false);
    ahrs->magnetic_disturbance =
        (((ahrs_update.sensor_status & NAVX_SENSOR_STATUS_MAG_DISTURBANCE) != 0)
             ? true
             : false);
    ...
    for (int i = 0; i < MAX_NUM_CALLBACKS; i++) {
      ...
      if (callback != NULL) {
        long system_timestamp =
            (long)(unit_cast<double>(Timer::GetFPGATimestamp()) * 1000);
        callback->timestampedDataReceived(system_timestamp, sensor_timestamp,
                                          ahrs_update,
    ...
    UpdateBoardStatus(ahrs_update.op_status, ahrs_update.sensor_status,
                      ahrs_update.cal_status, ahrs_update.selftest_status);
  ...
  void SetAHRSData(AHRSProtocol::AHRSUpdate& ahrs_update,
    ...
    ahrs->UpdateDisplacement(ahrs->world_linear_accel_x,
                             ahrs->world_linear_accel_y, ahrs->update_rate_hz,
                             ahrs->is_moving);
  ...
  void UpdateBoardStatus(uint8_t op_status, int16_t sensor_status,
    ...
    if (ahrs->op_status == NAVX_OP_STATUS_NORMAL) {
      ...
      if (op_status == NAVX_OP_STATUS_NORMAL) {
        ...
        if ((cal_status & NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) !=
            NAVX_CAL_STATUS_IMU_CAL_COMPLETE) {
          Tracer::Trace(
              "navX-Sensor startup initialization underway; startup "
              "calibration in progress.\n");
        } else {
          Tracer::Trace(
              "navX-Sensor startup initialization and startup calibration "
              "complete.\n");
        }
      }
    }

    /* Detect/report reset of yaw angle tracker upon transition to startup
     * calibration complete state. */
    if (((ahrs->cal_status & NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) !=
         NAVX_CAL_STATUS_IMU_CAL_COMPLETE) &&
        ((cal_status & NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) ==
         NAVX_CAL_STATUS_IMU_CAL_COMPLETE)) {
      Tracer::Trace("navX-Sensor onboard startup calibration complete.\n");
      /* Carefully, only reset the software yaw reset offset if calibration
       * completion upon */
      /* initial poweron or after board reset occurs. */
      if (poweron_init_completed ||
          ahrs->disconnect_startupcalibration_recovery_pending) {
        ahrs->disconnect_startupcalibration_recovery_pending = false;
        ahrs->yaw_angle_tracker->Init();
        Tracer::Trace(
            "navX-Sensor Yaw angle auto-reset to 0.0 due to startup "
            "calibration.\n");
      }
    }
  ...
  void DisconnectDetected() {
    /* Board disconnect may be caused by intermittent communication loss, or by
     * board reset. */
    /* Default status to error */
    ahrs->op_status = NAVX_OP_STATUS_ERROR;
    ahrs->sensor_status = 0; /* Clear all sensor status flags */
    /* Flag the need to watch for a startup calibration completion event upon
     * later reconnect. */
  ...
  bool IsOmniMountSupported() {
    return (((ahrs->capability_flags & NAVX_CAPABILITY_FLAG_OMNIMOUNT) != 0)
                ? true
                : false);
  }

  bool IsBoardYawResetSupported() {
    return (((ahrs->capability_flags & NAVX_CAPABILITY_FLAG_YAW_RESET) != 0)
                ? true
                : false);
  }

  bool IsDisplacementSupported() {
    return (((ahrs->capability_flags & NAVX_CAPABILITY_FLAG_VEL_AND_DISP) != 0)
                ? true
                : false);
  }

  bool IsAHRSPosTimestampSupported() {
    return (((ahrs->capability_flags & NAVX_CAPABILITY_FLAG_AHRSPOS_TS) != 0)
                ? true
                : false);
  }
};
...
AHRS::AHRS(frc::SPI::Port spi_port_id, uint32_t spi_bitrate,
 ...
 * @param update_rate_hz Custom Update Rate (Hz)
 */
AHRS::AHRS(frc::SerialPort::Port serial_port_id, AHRS::SerialDataType data_type,
           uint8_t update_rate_hz)
 ...
 *rate. <p> This constructor should be used if communicating via SPI. <p>
...
/**
 * Constructs the AHRS class using I2C communication and the default update
 *rate. <p> This constructor should be used if communicating via I2C. <p>
 * @param i2c_port_id I2C port to use
 */
 ...
 *rate, and returning processed (rather than raw) data. <p> This constructor
 *should be used if communicating via either TTL UART or USB Serial interface.
 *<p>
 * @param serial_port_id SerialPort to use
 */
AHRS::AHRS(frc::SerialPort::Port serial_port_id)
    : m_simDevice("navX-Sensor", serial_port_id) {
  SerialInit(serial_port_id, SerialDataType::kProcessedData,
...
void AHRS::ZeroYaw() {
  ...
  if (delta_time_since_last_yawreset_request <
    ...
    if ((successive_suppressed_yawreset_request_count %
         NUM_SUPPRESSED_SUCCESSIVE_YAWRESET_MESSAGES) == 1) {
      if (logging_enabled)
  ...
  if (IsCalibrating()) {
    ...
    if ((delta_time_since_last_yawreset_while_calibrating_request >
         SUPPRESSED_SUCESSIVE_YAWRESET_PERIOD_SECONDS)) {
      Tracer::Trace(
          "navX-Sensor Yaw Reset request ignored - startup calibration is "
...
bool AHRS::IsCalibrating() {
  return !((cal_status & NAVX_CAL_STATUS_IMU_CAL_STATE_MASK) ==
           NAVX_CAL_STATUS_IMU_CAL_COMPLETE);
}
 ...
 * sea-level barometric pressure [navX Aero only].  This value is in units of
 *meters. <p> NOTE:  This value is only valid sensors including a pressure
 ...
 *the Yaw Angle since the last known valid Compass Angle.  The last known valid
 *Compass Angle is updated whenever a Calibrated Compass Angle is read and the
 *sensor has recently rotated less than the Compass Noise Bandwidth (~2
 *degrees).
 ...
 * @return true if a magnetic disturbance is detected (or the magnetometer is
 *uncalibrated).
 */
...
/**
 * Each time new linear acceleration samples are received, this function should
 * be invoked. This function transforms acceleration in G to meters/sec^2, then
 * converts this value to Velocity in meters/sec (based upon velocity in the
 * previous sample).  Finally, this value is converted to displacement in
 * meters, and integrated.
 * @return none.
 */

void AHRS::UpdateDisplacement(float accel_x_g, float accel_y_g,
                              int update_rate_hz, bool is_moving) {
  integrator->UpdateDisplacement(accel_x_g, accel_y_g, update_rate_hz,
                                 is_moving);
}
 ...
 * The resulting velocities are not known to be very accurate.
 * @return Current Velocity (in meters/squared).
 */
float AHRS::GetVelocityX() {
  return (ahrs_internal->IsDisplacementSupported()
              ? velocity[0]
              : integrator->GetVelocityX());
}
...
/**
 * Returns true if Board-level yaw resets are enabled.  Conversely, returns
 * false if Software-based yaw resets are active.
 *
...
void AHRS::SPIInit(SPI::Port spi_port_id, uint32_t bitrate,
                   uint8_t update_rate_hz) {
  ...
  if (m_simDevice) {
    ...
    io = new RegisterIO(new RegisterIO_SPI(new SPI(spi_port_id), bitrate),
                        update_rate_hz, ahrs_internal, ahrs_internal);
  }
...
void AHRS::I2CInit(I2C::Port i2c_port_id, uint8_t update_rate_hz) {
  ...
  if (m_simDevice) {
    ...
    io = new RegisterIO(
        new RegisterIO_I2C(new I2C(i2c_port_id, NAVX_MXP_I2C_ADDRESS)),
        update_rate_hz, ahrs_internal, ahrs_internal);
  }
...
void AHRS::SerialInit(SerialPort::Port serial_port_id,
  ...
  if (m_simDevice) {
    ...
    io = new SerialIO(serial_port_id, update_rate_hz, processed_data,
                      ahrs_internal, ahrs_internal);
  }
 ...
 *degrees/sec).  NOTE:  this value is un-processed, and should only be accessed
 *by advanced users. Typically, rotation about the X Axis is referred to as
 *"Pitch".  Calibrated and Integrated Pitch data is accessible via the {@link
 *#GetPitch()} method. <p>
 ...
 *degrees/sec).  NOTE:  this value is un-processed, and should only be accessed
 *by advanced users. Typically, rotation about the T Axis is referred to as
 *"Roll".  Calibrated and Integrated Pitch data is accessible via the {@link
 *#GetRoll()} method. <p>
 * @return Returns the current rotation rate (in degrees/sec).
 */
 ...
 *degrees/sec).  NOTE:  this value is un-processed, and should only be accessed
 *by advanced users. Typically, rotation about the T Axis is referred to as
 *"Yaw".  Calibrated and Integrated Pitch data is accessible via the {@link
 *#GetYaw()} method. <p>
 * @return Returns the current rotation rate (in degrees/sec).
 */
...
/**
 * Returns the current raw (unprocessed) X-axis acceleration rate (in G).  NOTE:
 *this value is unprocessed, and should only be accessed by advanced users. This
 *raw value has not had acceleration due to gravity removed from it, and has not
 *been rotated to the world reference frame.  Gravity-corrected, world reference
 *frame-corrected X axis acceleration data is accessible via the {@link
 *#GetWorldLinearAccelX()} method. <p>
...
/**
 * Returns the current raw (unprocessed) Y-axis acceleration rate (in G).  NOTE:
 *this value is unprocessed, and should only be accessed by advanced users. This
 *raw value has not had acceleration due to gravity removed from it, and has not
 *been rotated to the world reference frame.  Gravity-corrected, world reference
 *frame-corrected Y axis acceleration data is accessible via the {@link
 *#GetWorldLinearAccelY()} method. <p>
 * @return Returns the current acceleration rate (in G).
 */
...
/**
 * Returns the current raw (unprocessed) Z-axis acceleration rate (in G).  NOTE:
 *this value is unprocessed, and should only be accessed by advanced users. This
 *raw value has not had acceleration due to gravity removed from it, and has not
 *been rotated to the world reference frame.  Gravity-corrected, world reference
 *frame-corrected Z axis acceleration data is accessible via the {@link
 *#GetWorldLinearAccelZ()} method. <p>
 * @return Returns the current acceleration rate (in G).
 */
...
/**
 * Returns the current raw (unprocessed) X-axis magnetometer reading (in
 *uTesla).  NOTE: this value is unprocessed, and should only be accessed by
 *advanced users.  This raw value has not been tilt-corrected, and has not been
 *combined with the other magnetometer axis data to yield a compass heading.
 *Tilt-corrected compass heading data is accessible via the {@link
 *#GetCompassHeading()} method. <p>
...
/**
 * Returns the current raw (unprocessed) Y-axis magnetometer reading (in
 *uTesla).  NOTE: this value is unprocessed, and should only be accessed by
 *advanced users.  This raw value has not been tilt-corrected, and has not been
 *combined with the other magnetometer axis data to yield a compass heading.
 *Tilt-corrected compass heading data is accessible via the {@link
...
/**
 * Returns the current raw (unprocessed) Z-axis magnetometer reading (in
 *uTesla).  NOTE: this value is unprocessed, and should only be accessed by
 *advanced users.  This raw value has not been tilt-corrected, and has not been
 *combined with the other magnetometer axis data to yield a compass heading.
 *Tilt-corrected compass heading data is accessible via the {@link
...
void AHRS::InitSendable(wpi::SendableBuilder& builder) {
  ...
  builder.AddDoubleProperty(
      "Value", [this]() { return GetYaw(); }, nullptr);
}
...
int AHRS::GetActualUpdateRate() {
  uint8_t actual_update_rate =
      GetActualUpdateRateInternal(GetRequestedUpdateRate());
