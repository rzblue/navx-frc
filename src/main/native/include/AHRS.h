...
#ifndef NAVX_FRC_SRC_MAIN_NATIVE_INCLUDE_AHRS_H_
#define NAVX_FRC_SRC_MAIN_NATIVE_INCLUDE_AHRS_H_
...
class AHRS : public frc::Gyro,
 ...
 public:
  ...
  /// Structure describing the current board orientation w/respect to the IMU
  /// axes of measurement.
  struct BoardYawAxis {
    /// Identifies one of the board axes.
    BoardAxis board_axis;
    /// true if axis is pointing up (with respect to gravity); false if pointing
    /// down.
    bool up;
  };
...
#define MAX_NUM_CALLBACKS 3
 ...
 private:
  void SPIInit(frc::SPI::Port spi_port_id, uint32_t bitrate,
               uint8_t update_rate_hz);
  void I2CInit(frc::I2C::Port i2c_port_id, uint8_t update_rate_hz);
  void SerialInit(frc::SerialPort::Port serial_port_id,
...
#endif  // NAVX_FRC_SRC_MAIN_NATIVE_INCLUDE_AHRS_H_
