...
class SerialIO implements IIOProvider {
    ...
    protected SerialPort resetSerialPort()
    {
        if (serial_port != null) {
    		...
    		Tracer.Trace("Closing " + (is_usb ? "USB " : "TTL UART ") + " serial port to communicate with navX-Sensor.\n"); 
            try {
                serial_port.close();
            } catch (Exception ex) {
    ...
    protected SerialPort getMaybeCreateSerialPort()
    {
        if (serial_port == null) {
            try {
        		Tracer.Trace("Opening " + (is_usb ? "USB " : "TTL UART ") + " serial port to communicate with navX-Sensor.\n"); 
        		...
        		Tracer.Trace("Error opening " + (is_usb ? "USB " : "TTL UART ") + " serial port to communicate with navX-Sensor.\n"); 
                serial_port = null;
            }
        }
        return serial_port;
    ...
    public void run() {
        ...
        while(!boardid_request_sent) {
	        try {
	            ...
	            //ex.printStackTrace();
	        	if(serial_port == null) {
	        		Tracer.Trace("Unable to open " + (is_usb ? "USB " : "TTL UART ") + " serial port to communicate with navX-Sensor.\n"); 
	        	} else {
	        		Tracer.Trace("Error sending navX-Sensor configuration request over " + (is_usb ? "USB " : "TTL UART ") + " serial port.\n");
	        		ex.printStackTrace();
	        	}
                Timer.delay(1.0);	            
	    		resetSerialPort();
