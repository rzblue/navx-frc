...
class SerialIO implements IIOProvider {
  ...
  public SerialIO(
      SerialPort.Port port_id,
      byte update_rate_hz,
      boolean processed_data,
      IIOCompleteNotification notify_sink,
      IBoardCapabilities board_capabilities) {
    this.serial_port_id = port_id;
    is_usb =
        ((port_id == SerialPort.Port.kUSB)
  ...
  protected SerialPort resetSerialPort() {
    if (serial_port != null) {
      ...
      Tracer.Trace(
          "Closing "
              + (is_usb ? "USB " : "TTL UART ")
              + " serial port to communicate with navX-Sensor.\n");
      try {
        serial_port.close();
      } catch (Exception ex) {
  ...
  protected void dispatchStreamResponse(IMUProtocol.StreamResponse response) {
    ...
    board_state.capability_flags =
        (short) (response.flags & ~IMUProtocol.NAV6_FLAG_MASK_CALIBRATION_STATE);
  ...
  protected int decodePacketHandler(byte[] received_data, int offset, int bytes_remaining) {
    ...
    if ((packet_length =
      ...
      notify_sink.setYawPitchRoll(ypr_update_data, sensor_timestamp);
            ...
            AHRSProtocol.decodeAHRSPosTSUpdate(
                received_data, offset, bytes_remaining, ahrspos_ts_update_data))
        > 0) {
      if (ahrspos_ts_update_data.op_status != 0) {
        notify_sink.setAHRSPosData(ahrspos_ts_update_data, ahrspos_ts_update_data.timestamp);
      }
    } else if ((packet_length =
            AHRSProtocol.decodeAHRSPosUpdate(
                received_data, offset, bytes_remaining, ahrspos_update_data))
        > 0) {
      if (ahrspos_ts_update_data.op_status != 0) {
        notify_sink.setAHRSPosData(ahrspos_update_data, sensor_timestamp);
      }
    } else if ((packet_length =
            AHRSProtocol.decodeAHRSUpdate(received_data, offset, bytes_remaining, ahrs_update_data))
        > 0) {
      if (ahrspos_ts_update_data.op_status != 0) {
        notify_sink.setAHRSData(ahrs_update_data, sensor_timestamp);
      }
    } else if ((packet_length =
            IMUProtocol.decodeGyroUpdate(received_data, offset, bytes_remaining, gyro_update_data))
        > 0) {
      notify_sink.setRawData(gyro_update_data, sensor_timestamp);
    } else if ((packet_length =
            AHRSProtocol.decodeBoardIDGetResponse(received_data, offset, bytes_remaining, board_id))
        > 0) {
  ...
  public void run() {
    ...
    AHRSProtocol.IntegrationControl integration_control_response =
        new AHRSProtocol.IntegrationControl();
    ...
    while (!boardid_request_sent) {
      try {
        ...
        cmd_packet_length =
            AHRSProtocol.encodeDataGetRequest(
                stream_command, AHRSProtocol.AHRS_DATA_TYPE.BOARD_IDENTITY, (byte) 0);
        serial_port.write(stream_command, cmd_packet_length);
        serial_port.flush();
        port_reset_count++;
        if (debug) {
        ...
        Tracer.Trace("navX-Sensor Connected " + (is_usb ? "via USB.\n" : "via TTL UART.\n"));
        ...
        if (serial_port == null) {
          ...
          Tracer.Trace(
              "Error sending navX-Sensor configuration request over "
                  + (is_usb ? "USB " : "TTL UART ")
                  + " serial port.\n");
          ex.printStackTrace();
        }
    ...
    while (!stop) {
      try {
        ...
        if (serial_port == null) {
          ...
          if (debug)
            Tracer.Trace("Initiating reset of serial port, as serial_port reference is null.\n");
          resetSerialPort();
          continue;
        }
        ...
        if (signal_transmit_integration_control) {
          ...
          cmd_packet_length =
              AHRSProtocol.encodeIntegrationControlCmd(
        ...
        if (bytes_read > 0) {
          ...
          while (i < bytes_read) {
                  ...
                  if (c.length > 0) {
                    ...
                    if (debug) {
                      SmartDashboard.putNumber(
                          "navX Partial Binary Packets", (double) partial_binary_packet_count);
                    }
                    continue;
                  }
                }
            ...
            byte[] received_data_copy = received_data.clone();
            ...
            if (packet_length > 0) {
              ...
              if (packet_length > 0) {
                Tracer.Trace("navX-Sensor Configuration Response Received.\n");
                packets_received++;
                if (!connect_reported) {
                  notify_sink.connectDetected();
                  connect_reported = true;
                ...
                if (debug) {
                  SmartDashboard.putNumber(
                      "navX Stream Responses", (double) stream_response_receive_count);
                }
              } else {
                packet_length =
                    AHRSProtocol.decodeIntegrationControlResponse(
                        received_data, i, bytes_remaining, integration_control_response);
                if (packet_length > 0) {
                  ...
                  i += packet_length;
                  if ((integration_control.action & AHRSProtocol.NAVX_INTEGRATION_CTL_RESET_YAW)
                      != 0) {
                    ...
                    if (!next_packet_start_found) {
                      ...
                      if ((bytes_remaining > 2)
                        ...
                        if (bytes_remaining >= pkt_len) {
                          ...
                          if (debug) {
                            SmartDashboard.putNumber(
                                "navX Discarded Bytes", (double) discarded_bytes_count);
                          }
                        } else {
          ...
          if (retransmit_stream_config
            ...
            cmd_packet_length =
                IMUProtocol.encodeStreamCommand(stream_command, update_type, update_rate_hz);
            try {
              Tracer.Trace("Retransmitting stream configuration command to navX-Sensor.\n");
              /* Ugly Hack.  This is a workaround for ARTF5478:           */
              /* (USB Serial Port Write hang if receive buffer not empty. */
              if (is_usb) {
                if (debug) Tracer.Trace("Resetting serial port via reset().\n");
                try {
                  serial_port.reset();
                } catch (RuntimeException ex2) {
                  /* Sometimes an unclean status exception occurs during reset(). */
          ...
          if (Timer.getFPGATimestamp() - last_data_received_timestamp > 1.0) {
            if (debug)
              Tracer.Trace(
                  "Initiating Serial Port Reset since no data was received in the last second.\n");
            resetSerialPort();
          }
        }
      } catch (RuntimeException ex) {
        ...
        if (debug) {
          SmartDashboard.putNumber(
              "navX Serial Port Timeout / Buffer Overrun", (double) timeout_count);
          SmartDashboard.putString("navX Last Exception", ex.getMessage() + "; " + ex.toString());
        }
        ex.printStackTrace();
        if (debug)
          Tracer.Trace("Initiating Serial Port Reset due to exception during Run() loop.\n");
        resetSerialPort();
      }
    }
  ...
  /**
   * Indicates whether the navX MXP is currently connected to the host computer. A connection is
   * considered established whenever a value update packet has been received from the navX MXP
  ...
  /**
   * Returns the count in bytes of data received from the navX MXP. This could can be useful for
  ...
  /**
   * Returns the count of valid update packets which have been received from the navX MXP. This
