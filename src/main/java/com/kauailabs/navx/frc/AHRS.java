...
public class AHRS implements Sendable, Gyro {
  ...
  /**
   * Constructs the AHRS class using I2C communication, overriding the default update rate with a
   * custom rate which may be from 4 to 200, representing the number of updates per second sent by
   * the sensor.
   *
   * <p>This constructor should be used if communicating via I2C.
   *
   * <p>Note that increasing the update rate may increase the CPU utilization.
   *
   * <p>
  ...
  public AHRS(I2C.Port i2c_port_id, byte update_rate_hz) {
    Tracer.Trace("Instantiating navX-Sensor on I2C Port %s.\n", i2c_port_id.toString());
    commonInit(update_rate_hz);
    if (m_simDevice != null) {
      io = new SimIO(update_rate_hz, io_complete_sink, m_simDevice);
    } else {
      io =
          new RegisterIO(
              new RegisterIO_I2C(new I2C(i2c_port_id, 0x32)),
              update_rate_hz,
              io_complete_sink,
              board_capabilities);
    }
  ...
  /**
   * Constructs the AHRS class using serial communication, overriding the default update rate with a
   * custom rate which may be from 4 to 200, representing the number of updates per second sent by
   * the sensor.
   *
   * <p>This constructor should be used if communicating via either TTL UART or USB Serial
   * interface.
   *
   * <p>Note that the serial interfaces can communicate either processed data, or raw data, but not
   * both simultaneously. If simultaneous processed and raw data are needed, use one of the
   * register-based interfaces (SPI or I2C).
   *
   * <p>Note that increasing the update rate may increase the CPU utilization.
   *
   * <p>
  ...
  public AHRS(SerialPort.Port serial_port_id, SerialDataType data_type, byte update_rate_hz) {
    Tracer.Trace("Instantiating navX-Sensor on Serial Port %s.\n", serial_port_id.toString());
    commonInit(update_rate_hz);
    if (m_simDevice != null) {
      io = new SimIO(update_rate_hz, io_complete_sink, m_simDevice);
    } else {
      ...
      io =
          new SerialIO(
              serial_port_id, update_rate_hz, processed_data, io_complete_sink, board_capabilities);
    }
  ...
  /**
   * Constructs the AHRS class using serial communication and the default update rate, and returning
   * processed (rather than raw) data.
   *
   * <p>This constructor should be used if communicating via either TTL UART or USB Serial
   * interface.
   *
